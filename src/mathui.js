...
export default class MathUI extends Plugin {
	...
	_initGuppy() {
		...
		if (!this.guppy) {
			...
			this.engine = this.guppy.engine;
			this.guppy.event( 'change', this._handlerMathInputChange );
		}
		this.guppy.activate();
		document.getElementById('guppy').focus()
		document.getElementById( 'input-guppy' ).addEventListener( 'change', ( e ) => {

			let latex = e.target.value;
			let xml = e.target.getAttribute( 'xml' );

			let fieldView = this.formView.mathInputView.fieldView;
			let displayButtonView = this.formView.displayButtonView;
			let saveButtonView = this.formView.saveButtonView;
			let mathView = this.formView.mathView;

			// fieldView.element.value = e.target.value

			if (fieldView.element != null) {
				let equationInput = latex.trim();
				...
				if (hasDelimiters( equationInput )) {
					...
					displayButtonView.isOn = params.display;
				}

				// Update preview view
				mathView.value = equationInput;

				if (equationInput && equationInput.length > 0) {
	...
	_handlerMathInputChange( data ) {
		var latex = data.target.engine.get_content( 'latex' );
		var xml = data.target.engine.get_content( 'xml' );
		console.log(xml);
		var evt = new CustomEvent( 'change' );
		var guppyInputStorage = document.getElementById( 'input-guppy' );
		guppyInputStorage.value = latex;
		guppyInputStorage.setAttribute( 'xml', xml );
		guppyInputStorage.dispatchEvent( evt );
	}
	...
	_createFormView() {
		...
		this.listenTo( formView, 'submit', () => {
			editor.execute( 'math', formView.equation, formView.displayButtonView.isOn, mathConfig.outputType, mathConfig.forceOutputType, formView.xml );
			this._closeFormView();
		} );
	...
	_addFormView() {
		...
		this._balloon.add( {
			view: this.formView, position: getBalloonPositionData( editor )
		} );
		...
		this.formView.xml = mathCommand.xml || '';
